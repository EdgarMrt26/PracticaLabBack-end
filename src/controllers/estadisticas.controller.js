...
export const totalVentasPorDia = async (req, res) => {
  try {
    const [result] = await pool2.query(
      ` SELECT DATE_FORMAT(t.fecha, '%Y-%m-') AS dia, SUM(hv.total_linea) AS total_ventas
...
export const clientesFrecuentes = async (req, res) => {
  try {
    ...
    if (result.length === 0) {
      return res.status(404).json({
        mensaje: 'No se encontraron clientes frecuentes.',
      });
    }
    res.json(result);
  } catch (error) {
...
export const clientesFrecuentesPorMes = async (req, res) => {
  try {
    ...
    if (result.length === 0) {
      return res.status(404).json({
        mensaje: 'No se encontraron clientes frecuentes por mes.',
      });
    }
    res.json(result);
  } catch (error) {
    return res.status(500).json({
      mensaje: 'Ha ocurrido un error al obtener los clientes frecuentes por mes.',
      error: error.message,
    });
  }
};
...
export const productosMasCompradosPorCliente = async (req, res) => {
  try {
    ...
    if (result.length === 0) {
      return res.status(404).json({
        mensaje: 'No se encontraron estadísticas de productos comprados por cliente.',
      });
    }
    res.json(result);
  } catch (error) {
    return res.status(500).json({
      mensaje: 'Ha ocurrido un error al obtener las estadísticas de productos comprados por cliente.',
      error: error.message,
    });
  }
};
...
export const categoriasMasCompradasPorCliente = async (req, res) => {
  try {
    ...
    if (result.length === 0) {
      return res.status(404).json({
        mensaje: 'No se encontraron estadísticas de categorías compradas por cliente.',
      });
    }
    res.json(result);
  } catch (error) {
    return res.status(500).json({
      mensaje: 'Ha ocurrido un error al obtener las estadísticas de categorías compradas por cliente.',
      error: error.message,
    });
  }
};
